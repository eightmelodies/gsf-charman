...
TODO: given a list of character names and the list of strategies, 1) determine current character to login and 2) handle logging off/on. 

Method get_character_currently_logged_in - For each character we manage, GET /characters/{name}/session and return the currently logged in character (is_logged_in is True). If more than one character is logged in, throw an exception (for now, RunetimeException, we'll handle this edge case later).

Method get_character_selected_for_login - For each character we manage, GET /characters{name} and valuate our layered strategy with the list of character data. Returns the first character in the list generated by evaluating our strategies. If this list is empty, throw an exception (for now RuntimeException, we'll handle this edge case later).
