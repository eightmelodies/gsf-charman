...
- Might be a good idea to associate actions to strategies. Eg., daily login should have the login action. This way you only login real quick to get your daily out of the way. As it is now, we'll run our startup scripts which just starts ebounty (this is still ok as worst case we get a hunt in and our daily LTEs out of the way before we logout). I might also want to handle chores like emptying containers of stuff that needs loresinging, etc.
...
- along these same lines, I probably need a way to signal the api that a character might be stuck and they should be excluded from login selection
- probably a good idea to break out the stats (lumnis, resource, etc.) into its own script i can run separately to still get data when i don't want my characters to be managed
...
I need to think about the API some more and where/when it makes sense to update fields like this. We definitely want charman.lic to send the is_logged_in state as it's the best indication that things are working correctly (vs from the perspective of the daemon where simplying starting the process doesn't necessarily indicate success).